...
const GumletPlayerComponent = () => {
  ...
  if (!drmToken)
    return <div>Unable to load video. Please try again later.</div>;
  ...
  const getVolume = async () => {
    if (playerRef.current)
      alert(`Volume: ${await playerRef.current.getVolume()}`);
  };
  const getDuration = async () => {
    if (playerRef.current)
      alert(`Duration: ${await playerRef.current.getDuration()} seconds`);
  };
  const getCurrentTime = async () => {
    if (playerRef.current)
      alert(
        `Current Time: ${await playerRef.current.getCurrentTime()} seconds`
      );
  };
  const getPlaybackRate = async () => {
    if (playerRef.current)
      alert(`Playback Rate: ${await playerRef.current.getPlaybackRate()}`);
  };
  ...
  return (
    <>
      <div style={{ padding: "10px" }}>
        <button style={{ marginRight: "10px" }} onClick={play}>
          Play
        </button>
        <button style={{ marginRight: "10px" }} onClick={pause}>
          Pause
        </button>
        <button style={{ marginRight: "10px" }} onClick={mute}>
          Mute
        </button>
        <button style={{ marginRight: "10px" }} onClick={unmute}>
          Unmute
        </button>
        <button style={{ marginRight: "10px" }} onClick={() => setVolume(50)}>
          Set volume to 50%
        </button>
        <button
        ...
        <button
          style={{ marginRight: "10px" }}
          onClick={() => setPlaybackRate(Number(1.5))}
        >
          Set playback rate to 2x
        </button>
        <button style={{ marginRight: "10px" }} onClick={getVolume}>
          Get volume
        </button>
        <button style={{ marginRight: "10px" }} onClick={getDuration}>
          Get duration
        </button>
        <button style={{ marginRight: "10px" }} onClick={getCurrentTime}>
          Get current time
        </button>
        <button style={{ marginRight: "10px" }} onClick={getPlaybackRate}>
          Get playback rate
        </button>
      </div>
